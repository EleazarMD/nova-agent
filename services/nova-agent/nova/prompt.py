...
def build_system_prompt(
    ...
    sections.append(
        ...
        "  - Knowledge Graph: homelab entities, relationships, infrastructure facts.\n"
        "  - LIAM: 48 scientific frameworks across 16 life dimensions.\n"
        "  READ: recall_memory → fetches preferences/identity.\n"
        ...
        "  WRITE: save_memory → stores confirmed facts (requires user confirmation).\n"
        "  WRITE: forget_memory → removes preferences.\n"
        ...
        "- **CIG** (Communications Intelligence Graph, port 8780): Email/calendar/contact analytics.\n"
        "  READ: query_cig → urgency, patterns, relationship health.\n"
        "  WRITE: Use hub_delegate(agent='hermes') for drafting/scheduling.\n"
        "- **AI Gateway** (port 8777): Model routing for all LLM calls. Routes to minimax, perplexity, qwen-vision, etc.\n"
        "- **NVIDIA NIM** (port 8006): Local GPU embeddings (nv-embedqa-e5-v5, 1024-dim). Powers semantic search.\n"
        "- **PostgreSQL** (port 5432): Primary database — conversations, messages, embeddings (pgvector).\n"
        "- **Neo4j** (port 7474/7687): Graph database — PCG entities, relationships, communities.\n"
    ...
    if tool_names:
        sections.append(
            ...
            "    hermes-heartbeat, tb-mbox-watcher, neo4j, postgresql, redis, nova-agent (18800/18803)\n"
            ...
            "    qwen-vision (8010, llama.cpp), openclaw-chrome (VNC 6080), code-server\n"
